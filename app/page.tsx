...
export default function DJRandomizer() {
  ...
  // Function to post trick to chat via secure API route
  const postTrickToChat = async (trick: Trick) => {
    try {
      const response = await fetch("/api/post-trick", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        console.error("[v0] Failed to post to chat:", await response.text())
      }
    } catch (error) {
